...
//        base_iterator_type eos;
    ...
    template<template <class, class> class Grammar, class DataType> 
    bool simple_parse(const std::string& input, DataType &result)
    {
        namespace qi = boost::spirit::qi;
        namespace ascii = boost::spirit::ascii;

        typedef std::string::const_iterator base_iterator_type;
        base_iterator_type start(input.begin());
        base_iterator_type end;
        qi::rule<base_iterator_type> skipper = ascii::space;
        Grammar<base_iterator_type, qi::rule<base_iterator_type> > g;
        
        return phrase_parse(start, end, g, skipper, result);
    }
